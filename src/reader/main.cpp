...
void initialize_views(ViewStack &view_stack, StateStore &state_store, SystemStyling &sys_styling, TokenViewStyling &token_view_styling, TaskQueue &task_queue, std::filesystem::path epub_file)
{
...
if (!epub_file.empty() && std::filesystem::exists(epub_file))
{
    // Check if there is a saved state for the specific book
    if (state_store.get_current_book_path() == epub_file)
    {
        // Load the book using the saved state for this EPUB
        load_book(state_store.get_current_book_path().value());
    }
    else
    {
        // If no state exists, load the book from scratch
        load_book(epub_file);
    }
}
else if (state_store.get_current_book_path())
{
    // Load the last opened book
    load_book(state_store.get_current_book_path().value());
}
...
int main(int argc, char *argv[])
{
    ...
    // Handle the command-line argument for the EPUB file
    std::filesystem::path epub_file;
    if (argc > 1) {
        epub_file = argv[1]; // Get the passed EPUB file path
    }

    initialize_views(view_stack, state_store, sys_styling, token_view_styling, task_queue, epub_file);
